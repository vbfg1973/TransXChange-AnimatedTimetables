...
-- and 'roadnodes' and that they are located within a schema named 'public'.
...
CREATE INDEX roadnodes_id_idx ON public.roadnodes USING btree (id); 
CREATE INDEX roadnodes_identifier_idx ON public.roadnodes USING btree (identifier); 
 
CREATE INDEX roadlinks_id_idx ON public.roadlinks USING btree (id); 
CREATE INDEX roadlinks_identifier_idx ON public.roadlinks USING btree (identifier); 
...
SELECT COUNT
	( * ) 
FROM
	( SELECT id, ROW_NUMBER () OVER ( PARTITION BY identifier ORDER BY id ) AS rnum FROM public.roadnodes ) t 
WHERE
	t.rnum > 1;

DELETE FROM public.roadnodes
WHERE id IN (SELECT id
              FROM (SELECT id,
                             ROW_NUMBER() OVER (partition BY identifier ORDER BY id) AS rnum
                     FROM public.roadnodes) t
              WHERE t.rnum > 1);
...
SELECT COUNT
	( * ) 
FROM
	( SELECT id, ROW_NUMBER () OVER ( PARTITION BY identifier ORDER BY id ) AS rnum FROM public.roadlinks ) t 
WHERE
	t.rnum > 1;
	

DELETE FROM public.roadlinks
WHERE id IN (SELECT id
              FROM (SELECT id,
                             ROW_NUMBER() OVER (PARTITION BY identifier ORDER BY id) AS rnum
                     FROM public.roadlinks) t
              WHERE t.rnum > 1);
...
DROP INDEX public.roadlinks_id_idx; 
DROP INDEX public.roadlinks_identifier_idx; 


-- add souce and target columns
ALTER TABLE public.roadlinks
    ADD COLUMN source integer, 
		ADD COLUMN target integer;

-- populate source and target fields using unique id from roadnodes table
UPDATE public.roadlinks l SET source = n.id FROM public.roadnodes n WHERE
l.startnode = n.identifier;

UPDATE public.roadlinks l SET target = n.id FROM public.roadnodes n WHERE
...
UPDATE public.roadlinks SET rcost_len = cost_len;
...
UPDATE public.roadlinks SET speed_mph =
    CASE
...
CREATE INDEX roadlinks_source_idx ON public.roadlinks USING btree(source);
CREATE INDEX roadlinks_target_idx ON public.roadlinks USING btree(target);

-- index id
CREATE INDEX roadlinks_id_idx ON public.roadlinks USING btree (id);

-- spatial index
CREATE INDEX roadlinks_geom_idx ON public.roadlinks USING gist (geom);
...
-- clean-up the table
VACUUM (ANALYZE, VERBOSE) public.roadlinks;
